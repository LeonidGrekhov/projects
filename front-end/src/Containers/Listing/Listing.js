...
class Listing extends Component {
  ...
  componentDidMount = () => {
    if (debug) {
      this.setState({bookData: bookJson.data, listData: listingJson.data, renderReady: true});
    } else {
      ListingAPI.getBookInfo(this.state.bid).then( 
        bookData => ListingAPI.getListingInfo(this.state.lid).then (
          listData => this.setState({bookData, listData, renderReady: true})
        )
      )
    }
  }
  ...
  addToCart = () => (<>
    <button className="btn btn-warning float-right mr-5"><h4><i className="fa fa-shopping-cart" /> Add To Cart</h4></button>
  </>)
  ...
  <div className="row my-3">
    ...
    <div className="row">
            ...
            <div className="col-1">
            <Generics.Body.RatingStar rating={this.state.listData.rating} dimension={12} />
            </div>
        </div>
